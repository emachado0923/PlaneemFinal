    ...
    public function createWordDocx(Request $request)
    {
       // $wordTest = new \PhpOffice\PhpWord\PhpWord();
        ...
        if($Propuesta == 'Propuesta_valor'){
              ...
              ->first();
               $seccion->setStyle('size' , 20);
               $seccion->addText('Propuesta_valor');

            /*$fuente=$seccion->setBold(true);
            $fuente=$seccion->setName('Tahoma');
            $fuente=$seccion->setSize(16);
            $fuente=$seccion->getColor('9F81F7');*/


          //  $Propuesta_valor = $seccion->addText($request->get('Propuesta_valor'),array('name'=>'Arial','size' => 20,'bold' => true));
...
/*
        ...
        $objWriter = \PhpOffice\PhpWord\IOFactory::createWriter($documento, 'Word2007');
         $objWriter->save('Planeem.docx');
        return response()->download(storage_path('Planeem.docx'));


       // return response()->download(public_path('Planeem.docx'));
